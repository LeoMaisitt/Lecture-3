...
filename=paste("WorldData.xlsx")

for (i in 1 : length(datalist))
{
  write.xlsx(datalist[[i]], filename, sheetName=paste(continents[i],years[i]), append=TRUE)
}

for (i in 1 : length(datalist))
{
  write.xlsx(datalist[[i]],'WorldData.xlsx', sheetName=paste(continents[i],years[i]), append=TRUE)
}

map2(.x=continents,
     .y=years,
     .f=~write.xlsx(datalist,'WorldData.xlsx', sheetName=paste(.x,.y), append=TRUE))